...
template <typename KeyT, typename OffT = uint16_t>
static void dump_leaf_csv_device_row(const uint8_t* d_rows,
                                     size_t count,
                                     size_t key_offset_bytes,
                                     size_t offset_offset_bytes,
                                     size_t row_stride_bytes,
                                     const char* csv_path = "zzz_static_tree_leaves_rows.csv")
{
    if (count == 0) return;
    const size_t stride = row_stride_bytes ? row_stride_bytes : (sizeof(KeyT) + sizeof(OffT));
    const size_t bytes  = count * stride;

    std::vector<uint8_t> h(bytes);
    check_cuda(cudaMemcpy(h.data(), d_rows, bytes, cudaMemcpyDeviceToHost), "Memcpy leaf rows");

    std::ofstream os(csv_path);
    if (!os) throw std::runtime_error(std::string("cannot open file: ") + csv_path);

    os << "leaf,offset\n";
    for (size_t i = 0; i < count; ++i) {
        const uint8_t* row = &h[i * stride];
        KeyT k{}; OffT off{};
        std::memcpy(&k,   row + key_offset_bytes,    sizeof(KeyT));
        std::memcpy(&off, row + offset_offset_bytes, sizeof(OffT));
        os << k << ',' << static_cast<uint64_t>(off) << '\n';
    }
}

template <typename KeyT, typename OffT = uint16_t>
static void dump_leaf_csv_device_cols(const KeyT* d_keys,
                                      const OffT* d_offsets,
                                      size_t count,
                                      const char* csv_path = "zzz_static_tree_leaves.csv")
{
    if (count == 0) return;
    std::vector<KeyT> hk(count);
    std::vector<OffT> ho(count);
    check_cuda(cudaMemcpy(hk.data(), d_keys,   count * sizeof(KeyT), cudaMemcpyDeviceToHost), "Memcpy leaf keys");
    check_cuda(cudaMemcpy(ho.data(), d_offsets, count * sizeof(OffT), cudaMemcpyDeviceToHost), "Memcpy leaf offsets");

    std::ofstream os(csv_path);
    if (!os) throw std::runtime_error(std::string("cannot open file: ") + csv_path);

    os << "leaf,offset\n";
    for (size_t i = 0; i < count; ++i) os << hk[i] << ',' << static_cast<uint64_t>(ho[i]) << '\n';
}


template <typename key_type>
static void dump_leaf_csv_device(const uint8_t* d_sorted_keys,
                                 size_t key_count,
                                 size_t key_offset_bytes,
                                 ...
                                 const char* csv_path = "zzz_static_tree_leaves.csv")
{
    ...
    const size_t row = (key_stride_bytes ? key_stride_bytes : sizeof(key_type));
    const size_t bytes = key_count * row;

    std::vector<uint8_t> h_rows(bytes);
    check_cuda(cudaMemcpy(h_rows.data(), d_sorted_keys, bytes, cudaMemcpyDeviceToHost), "Memcpy leaves");
    ...
    for (size_t i = 0; i < key_count; ++i) {
        key_type v{};
        std::memcpy(&v, &h_rows[i * row + key_offset_bytes], sizeof(key_type));
        os << v << '\n';
    }
}

template <typename key_type>
static void dump_nodes_csv_device(const key_type* d_nodes,
                                  size_t total_nodes,
                                  size_t node_size,
                                  const char* csv_path = "zzz_static_tree_nodes.csv")
{
    if (total_nodes == 0) return;

    const size_t count = total_nodes * node_size;
    std::vector<key_type> h(count);
    check_cuda(cudaMemcpy(h.data(), d_nodes, count * sizeof(key_type), cudaMemcpyDeviceToHost), "Memcpy nodes");

    std::ofstream os(csv_path);
    if (!os) throw std::runtime_error(std::string("cannot open file: ") + csv_path);

    // node_index,slot,value (one row per slot)
    os << "node,slot,value\n";
    for (size_t n = 0; n < total_nodes; ++n) {
        const size_t base = n * node_size;
        for (size_t s = 0; s < node_size; ++s) {
            os << n << ',' << s << ',' << h[base + s] << '\n';
        }
    }
}
...
template <typename key_type, uint16_t node_size_log>
GLOBALQUALIFIER
void build_static_tree_kernel(
        const uint8_t* sorted_keys,
        size_t key_count,
        size_t key_offset_bytes,
        size_t key_stride_bytes,
    ...
    const smallsize byte_offset = load_offset * key_stride_bytes + key_offset_bytes;
    ...
    bool exists = load_offset < key_count - 1;
...
template <typename key_type,
          bool use_row_layout,
          typename SmallT = uint16_t>
void after_tree_built_dump_csvs(const cuda_buffer<uint8_t>& sorted_key_offset_pairs_buffer,
                                const cuda_buffer<key_type>&  sorted_keys_buffer,
                                const cuda_buffer<SmallT>&    sorted_offsets_buffer,
                                size_t base_entries_count,
                                const cuda_buffer<key_type>&  static_tree_buffer,
                                const tree_metadata&          metadata,
                                size_t node_size)
{
    try {
        if constexpr (use_row_layout) {
            dump_leaf_csv_device_row<key_type, SmallT>(
                /*d_rows*/ sorted_key_offset_pairs_buffer.ptr(),
                /*count*/  base_entries_count,
                /*key_off*/ 0,
                /*off_off*/ sizeof(key_type),
                /*stride */ sizeof(key_type) + sizeof(SmallT),
                "zzz_static_tree_leaves_rows.csv");
        } else {
            dump_leaf_csv_device_cols<key_type, SmallT>(
                /*d_keys*/    sorted_keys_buffer.ptr(),
                /*d_offsets*/ sorted_offsets_buffer.ptr(),
                /*count*/     base_entries_count,
                "zzz_static_tree_leaves.csv");
        }

        dump_nodes_csv_device<key_type>(
            static_tree_buffer.ptr(), metadata.total_nodes, node_size, "zzz_static_tree_nodes.csv");
...
void build_static_tree(
    ...
    metadata.total_nodes = 0;
    {
        ...
        size_t nodes_on_current_level = SDIV(key_count, node_size);
        ...
        nodes_on_current_level = SDIV(key_count, node_size);
    ...
    // build tree
    {
        ...
        build_static_tree_kernel<key_type, node_size_log><<<nb, MAXBLOCKSIZE>>>
            (sorted_keys, key_count, key_offset_bytes, key_stride_bytes, metadata, static_tree_buffer_out.ptr());
    }
    ...
    //constexpr size_t node_size = size_t(1) << node_size_log;
    //after_tree_built_dump_csvs<key_type, use_row_layout, smallsize>(
     //   /*rows buffer*/    sorted_key_offset_pairs_buffer,
     //   /*keys buffer*/    sorted_keys_buffer,
     //   /*offsets buffer*/ sorted_offsets_buffer,
     //   /*count*/          base_entries_count,
    ...
    // Print metadata to stdout (very end)
    std::printf("Static tree metadata:\n");
    std::printf("  level_count: %zu\n", static_cast<size_t>(metadata_out.level_count));
    std::printf("  total_nodes: %zu\n", static_cast<size_t>(metadata_out.total_nodes));
    std::printf("  nodes_on_level: [");
    for (size_t i = 0; i < metadata_out.level_count; ++i) {
        std::printf("%zu%s", static_cast<size_t>(metadata_out.nodes_on_level[i]),
                    (i + 1 == metadata_out.level_count) ? "" : ", ");
    }
    std::printf("]\n");
}
...
smallsize static_tree_search(
    ...
    smallsize local_entries_offset = 0;
     ...
     if(tile_id==0 && tid==0) {
        printf("Starting static_tree_search for lookup key %u\n", lookup); // DEBUG
        printf("Tree metadata: level_count=%u\n", metadata.level_count); // DEBUG
        printf("Tree metadata: nodes_on_level="); // DEBUG
        for (smallsize i = 0; i < metadata.level_count; ++i) {
            printf("%u ", metadata.nodes_on_level[i]); // DEBUG
        }
        printf("\n"); // DEBUG
    }
    ...
    for (smallsize level = 0; level < metadata.level_count; ++level) {
        ...
        for (smallsize i = 0; i < cg_factor; ++i) {
            ...
            if (can_load) {
                if (shmem_tree != nullptr && caching_enabled && offset < cached_entries_count) {
                    printf("Loading from shared memory at offset %u\n", offset); // DEBUG
                    entry = shmem_tree[offset];
                } else {
                    if(tid==0) printf("Loading from static tree at offset %u tile_id %u\n", offset, tile_id); // DEBUG
                    entry = tree[offset];
                    if(tid==0)printf("Loaded entry: %u ,tile_id %u \n", entry, tile_id); // DEBUG
...
private:
    ...
    static constexpr uint16_t shared_memory_occupancy_class= shared_memory_occupancy;
...
public:
    ...
    void build(const key_type* keys, size_t size, size_t max_size, size_t available_memory_bytes, double* build_time_ms, size_t* build_bytes) {
        ...
        max_shared_memory_bytes = interpolate_shmem_bytes(shared_memory_occupancy_class);
        ...
        build_static_tree<key_type, node_size_log>(
                (const uint8_t*) sorted_keys_buffer.raw_ptr, size, 0, sizeof(key_type),
