...
probe:;
                ...
                if (active_range_end > active_range_start) {
                    ...
                    const bool sort_probes = true;  // toggle
                    if (sort_probes) {
                         // 1) Sort probe keys on device (keys-only)
                        timer.start();
                        auto sorted = key_only_sort_device<key_type>(probe_keys_buffer, probe_size, /*stream*/0);
                        timer.stop();
                        sort_time_ms = timer.time_ms();
                        std::cerr << "      Sorted the Probe Keys in " << sort_time_ms << " ms" << std::endl;
                        //probe_keys = sorted.sorted_keys;
                         //std::cerr << "      Sorted the Probe Keys " << std::endl;
                    ...
                    index.index_layer_lookups(sorted.sorted_keys.ptr(), probe_size, 0);
                    timer.stop();
