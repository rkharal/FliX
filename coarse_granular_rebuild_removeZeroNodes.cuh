...
template <typename key_type>
GLOBALQUALIFIER void count_nodes_per_bucket_kernel_previous(
    ...
    smallsize *nodes_per_bucket)
{
    ...
    // Initialize count
    size_t count = 0;  // Start with 0, as we check node size before counting
    ...
    while (next_offset != 0)
    {
        ...
        // Extract the node size
        smallsize current_node_size = cg::extract<smallsize>(curr_node, 0); // Assuming node size is stored at offset 0
        
        // Count only if node size is greater than 0
...
for (smallsize i = 0; i < node_count_for_this_partition;)
    {   
       if (size_currnode == 0) {
            continue; //ignore zero nodes.. do not increment i
        }
        ...
        size_currnode = cg::extract<smallsize>(curr_node, sizeof(key_type));
        i++;
