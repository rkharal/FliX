    ...
    size_t runs)
{
    ...
    std::vector<update_configuration> test_configuration_options{
        ...
        {"batches", 25, 26, 8, 200, 200, false, true},
    };
...
#pragma message "PRINT_GENERATED_HYBRID_KEYS=YES"
        ...
        for (size_t run = 0; run < runs; ++run)
        {
            ...
            try
            {
                ...
                //--- FOR BASELINES ---- index.build(insert_delete_keys_buffer.ptr(), active_range_end - active_range_start, key_generation_size, available_bytes_for_index, nullptr, nullptr);
...
probe:;
                // ===============================
                // Call-site integration (drop-in)
                // ===============================

                // FIRST APPROACH WITH INDEX AND BUCKET LAYER and ORGINAL STAT TRREE
                /*
                if (active_range_end > active_range_start)
                {
                    ...
                    if (sort_probes)
                    {
                        ...
                        scatter_sorted_to_original<smallsize>(tmp_results_sorted,
                                                              ...
                                                              result_buffer,
                                                              probe_size, 0);
                       probe_time_ms = index_layer_time_ms + bucket_layer_time_ms + sort_time_ms;
                       cudaDeviceSynchronize();
                       C2EX
                    }
                    else
                ...
                */
                ...
                if (active_range_end > active_range_start)
                {
                    ...
                    if (sort_probes)
                    {
                        ...
                        //-------> index.lookups_static_tree(d_sorted_keys.ptr(),
                        //                         tmp_results_sorted.ptr(),
                        //                       probe_size, 0);
#ifdef BASELINES
#pragma message "BASELINES SORTED Lookups"
                ...
                {
                    std::cerr << "Sorting first miss probes of size " << active_range_start << std::endl;
...
#pragma message "Using BASELINE SORTED LOOKUP for MISS CHECKS"
                    ...
                    if (active_range_end < key_generation_size)
                    {
                        ...
                        const bool sort_probes = true;
                        smallsize probe_size_misses = static_cast<smallsize>(key_generation_size - active_range_end);
                         if (sort_probes && probe_size_misses)
                        {
                            // Persistent/reused buffers across iterations are recommended; local here for clarity
                            static cuda_buffer<key_type> d_sorted_keys;
...
#pragma message "Using BASELINE SORTED LOOKUP for MISS 2 CHECKS"
                        ...
                        /* PREVIOUS WITH STATIC TREE NON SORTED
                          // check if keys before the inserted range are all misses
                        if (active_range_start > 0)
                        {
                            probe_keys_buffer.upload(generated_keys.data(), active_range_start);
                            result_buffer.zero();
                            index.lookups_static_tree(probe_keys_buffer.ptr(), result_buffer.ptr(), active_range_start, 0);
                        ...
                        if (active_range_end < key_generation_size)
                        {
                            ...
                            index.lookups_static_tree(probe_keys_buffer.ptr(), result_buffer.ptr(), key_generation_size - active_range_end, 0);
                        ...
                        */
                        ...
                        // Here
                        std::cerr << "--Probe Time Only: -> " << probe_time_ms << " ms" << std::endl;
