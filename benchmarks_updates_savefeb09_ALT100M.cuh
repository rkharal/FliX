...
void assert_ceiling_results_include_range(const std::vector<key_type>& a_keys,
                            ...
                            const char* gen_name = "generated_keys")
{
    ...
    const size_t n = generated_keys.size();

    const bool has_include = !(include_start == 0 && include_end == 0);
    if (has_include)
    {
        include_start = std::min(include_start, n);
        include_end = std::min(include_end, n);
        if (include_end < include_start) std::swap(include_start, include_end);
    }

    // Range 1: [0, d_end_point)
    std::vector<key_type> sorted_primary(generated_keys.begin(),
                                         generated_keys.begin() + d_end_point);
    std::sort(sorted_primary.begin(), sorted_primary.end());
    sorted_primary.erase(std::unique(sorted_primary.begin(), sorted_primary.end()),
                         sorted_primary.end());

    if (sorted_primary.empty())
    {
        std::cerr << "ASSERT FAIL: " << gen_name << " primary[0:" << d_end_point << ") is empty\n";
        std::abort();
    }

    // Range 2 (optional): [include_start, include_end)
    std::vector<key_type> sorted_include;
    if (has_include && include_start < include_end)
    {
        sorted_include.assign(generated_keys.begin() + include_start,
                              generated_keys.begin() + include_end);
        std::sort(sorted_include.begin(), sorted_include.end());
        sorted_include.erase(std::unique(sorted_include.begin(), sorted_include.end()),
                             sorted_include.end());
        // It is OK if include becomes empty; we just won't find ceilings there.
    }
    ...
    for (size_t i = 0; i < a_keys.size(); ++i)
    {
        ...
        // successor/ceiling in primary
        key_type cand_primary{};
        bool has_cand_primary = false;
        {
            auto it = std::lower_bound(sorted_primary.begin(), sorted_primary.end(), query);
            if (it != sorted_primary.end())
            {
                cand_primary = *it;
                has_cand_primary = true;
            }
        }

        // successor/ceiling in include
        key_type cand_include{};
        bool has_cand_include = false;
        if (!sorted_include.empty())
        {
            auto it = std::lower_bound(sorted_include.begin(), sorted_include.end(), query);
            if (it != sorted_include.end())
            {
                cand_include = *it;
                has_cand_include = true;
            }
        }

        if (!has_cand_primary && !has_cand_include)
        {
            continue; // preserve your existing behavior
        }

        const key_type expected =
            has_cand_primary && has_cand_include ? std::min(cand_primary, cand_include)
            : has_cand_primary                  ? cand_primary
                                                : cand_include;

        const key_type got = b_results[i];
        const bool skip = (got == 0);
        ...
        if (got != expected && !skip)
        {
            ...
            if (has_include)
            {
                std::cerr << " include=[" << include_start << ":" << include_end << ")";
            }
            else
            {
                std::cerr << " include=[none]";
            }

            std::cerr << " primary_min=" << sorted_primary.front()
                      << " primary_max=" << sorted_primary.back();

            if (!sorted_include.empty())
            {
                std::cerr << " include_min=" << sorted_include.front()
                          << " include_max=" << sorted_include.back();
            }
...
template <typename key_type>
void assert_ceiling_results_exclude(const std::vector<key_type>& a_keys,
                            const std::vector<key_type>& b_results,
                            const std::vector<key_type>& generated_keys,
                            size_t d_end_point,
                            ...
                            const char* gen_name = "generated_keys")
{
    ...
    const bool has_exclusion = !(exclude_start == 0 && exclude_end == 0);

    // Intersect exclusion (index-based) with [0, d_end_point)
    if (has_exclusion)
    {
        if (exclude_start > d_end_point) exclude_start = d_end_point;
        if (exclude_end > d_end_point) exclude_end = d_end_point;
        if (exclude_end < exclude_start) std::swap(exclude_start, exclude_end);
    }

    std::vector<key_type> sorted_generated_set;
    sorted_generated_set.reserve(d_end_point);

    for (size_t i = 0; i < d_end_point; ++i)
    {
        if (has_exclusion && i >= exclude_start && i < exclude_end)
            continue;
        sorted_generated_set.push_back(generated_keys[i]);
    }
    ...
    {
        std::cerr << "ASSERT FAIL: " << gen_name << "[0:" << d_end_point << ") is empty after exclusion "
                  << (has_exclusion ? ("[" + std::to_string(exclude_start) + ":" + std::to_string(exclude_end) + ")") : std::string("[none]"))
                  << "\n";
        std::abort();
    }
    ...
    for (size_t i = 0; i < a_keys.size(); ++i)
    {
        ...
        // >= query (ceiling), but only among allowed generated keys
        ...
        {
            continue; // keep your existing behavior
        }
        ...
        if (got != expected && !skip)
        {
            std::cerr << "ASSERT FAIL: ceiling mismatch at i=" << i
                      ...
                      << " " << b_name << "=" << got
                      << " d_end_point=" << d_end_point
                      << " exclude=[" << (has_exclusion ? exclude_start : 0) << ":" << (has_exclusion ? exclude_end : 0) << ")"
...
template <typename key_type>
void Draw_Miss_Probes_From_Two_Ranges(
    size_t size,
    const std::vector<key_type>& build_keys,
    size_t end_build_range,     // sample from [end_build_range, gen_key_range)
    size_t gen_key_range,       // exclusive upper bound
    size_t active_range_start,  // exclude [active_range_start, active_range_end) unless both are 0
    ...
    std::vector<smallsize>& expected_result)
{
    ...
    const size_t n = build_keys.size();

    end_build_range = std::min(end_build_range, n);
    gen_key_range = std::min(gen_key_range, n);

    probe_keys.resize(size);
    expected_result.resize(size);

    if (end_build_range >= gen_key_range)
    {
        return;
    }

    const size_t start = end_build_range;
    const size_t end = gen_key_range;

    const bool has_exclusion = !(active_range_start == 0 && active_range_end == 0);
    ...
    if (has_exclusion)
    {
        active_range_start = std::min(active_range_start, n);
        active_range_end = std::min(active_range_end, n);

        // Intersect exclusion with [start, end)
        excl_s = std::clamp(active_range_start, start, end);
        excl_e = std::clamp(active_range_end, start, end);

        // If exclusion is empty or inverted, treat as no exclusion.
        if (excl_e <= excl_s)
        {
            excl_s = start;
            excl_e = start;
    ...
    // A: [start, excl_s)
    // B: [excl_e, end)
    const size_t len_a = (excl_s > start) ? (excl_s - start) : 0;
    const size_t len_b = (end > excl_e) ? (end - excl_e) : 0;

    const size_t total_allowed = has_exclusion ? (len_a + len_b) : (end - start);
    ...
    std::uniform_int_distribution<size_t> dist(0, total_allowed - 1);
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        if (!has_exclusion)
        {
            index = start + dist(gen);
        }
        else
        {
            const size_t r = dist(gen);
            if (r < len_a)
            {
                index = start + r;
            }
            ...
            {
                index = excl_e + (r - len_a);
            }
        }

        probe_keys[i] = build_keys[index];
        expected_result[i] = std::numeric_limits<smallsize>::max(); // miss sentinel
    }
}


template <typename key_type>
void Draw_Hit_Probes_From_Two_Ranges(
    size_t size,
    const std::vector<key_type>& build_keys,
    size_t end_build_range,            // range (i): [0, end_build_range)
    size_t active_range_start,         // range (ii): [active_range_start, active_range_end)
    size_t active_range_end,
    bool start_expected_offset_at_zero,
    std::vector<key_type>& probe_keys,
    std::vector<smallsize>& expected_result)
{
    // Adjustable mix: 0.25 => 1 out of 4 probes from active range (ii), 3 out of 4 from range (i).
    const double kFractionFromActiveRange = 0.25;
    ...
    if (end_build_range > n) end_build_range = n;
    if (active_range_start > n) active_range_start = n;
    if (active_range_end > n) active_range_end = n;

    const bool has_build_range = (end_build_range > 0);
    const bool has_active_range = (active_range_end > active_range_start);

    probe_keys.resize(size);
    expected_result.resize(size);

    if (!has_build_range && !has_active_range)
    {
        // Nothing to draw from; leave vectors sized, but contents unspecified.
        // If you prefer hard failure: throw std::invalid_argument("Both ranges are empty.");
        return;
    }

    std::uniform_int_distribution<size_t> build_dist(
        0, has_build_range ? (end_build_range - 1) : 0);

    std::uniform_int_distribution<size_t> active_dist(
        has_active_range ? active_range_start : 0,
        has_active_range ? (active_range_end - 1) : 0);

    std::bernoulli_distribution pick_active(kFractionFromActiveRange);

    for (size_t i = 0; i < size; ++i)
    {
        const bool want_active = pick_active(gen);

        size_t index = 0;
        bool from_active = false;

        if (want_active && has_active_range)
        {
            index = active_dist(gen);
            from_active = true;
        }
        else if (has_build_range)
        {
            index = build_dist(gen);
            from_active = false;
        }
        else
        {
            // Build range missing; must draw from active.
            index = active_dist(gen);
            from_active = true;
        }
        ...
        if (start_expected_offset_at_zero)
        {
            expected_result[i] = from_active
                ? static_cast<smallsize>(index - active_range_start)
                : static_cast<smallsize>(index); // build-range starts at 0
        }
        else
        {
            expected_result[i] = static_cast<smallsize>(index);
        }
    }
}
    ...
    size_t runs)
{
    ...
    std::vector<update_configuration> test_configuration_options{
        ...
        {"batches", init_build_size_log, init_probe_size_log, rounds, 100, 100, true, true},
    };
    ...
    for (size_t tci = 0; tci < test_configuration_options.size(); ++tci)
    {
        ...
        // --> Adjust
       // size_t build_size = (size_t{1} << tc.build_size_log) + 333; // 10333+ 100;
        ...
        //--> Adjust
       // ----  size_t probe_size = (size_t{1} << tc.probe_size_log) + 10333; //10333
        size_t probe_size = 99999999 +2; 
...
#pragma message "COMBINE_INSERT_DELETE=NO"
...

            {
                ...
                //          << " -> This Round Updates starts at [" << active_range_end << "]" << std::endl;
                  ...
                  std::cerr << "----> Update step " << step << std::endl; // ": Active Range [" << active_range_start << ", " << active_range_end << ")"
                        //  << " -> This Round Updates starts at [" << active_range_end << "]" << std::endl;
                ...
                if constexpr (supports_updates)
                {
                    if (did_delete_this_step && !last_deleted_keys.empty())
                    {
                        ...
                        std::cerr << "    --Post-Delete Miss-Probe for " << del_probe_size << " keys..." << std::endl;
...
#pragma message "REBUILD_ON=YES"
            ...
            probe:;
                ...
                if (active_range_end > active_range_start)
                {
                    ...
                    // decide hits for rounds of inserts or deletes
                    // note: for probe following insert round we have build keys and one batch of inserts to draw hits keys from
                    // for probe following delete round we have build keys Only to draw hits keys from
                    smallsize range_start = do_insert? active_range_end - insert_batch_size : 0;
                    smallsize range_end = do_insert? active_range_end : 0;
                    
                    Draw_Hit_Probes_From_Two_Ranges(
                        probe_size,
                        generated_keys,
                        build_size,
                        range_start,
                        ...
                        std::cerr << "        - Probe Inner Range for Hits: [" << range_start << ", " << range_end << ")\n";  
...
#pragma message "Using REGULAR CGRXU LOOKUPS"
                        ...
                        if (PERFORM_SUCCESSOR_PROBES)
                        {
                            ...
                            tmp_results_sorted_keys.zero();
                                                        //--cuda_buffer<key_type> tmp_results_sorted_keys;
                            //--tmp_results_sorted_keys.alloc(probe_size);
                            //--tmp_results_sorted_keys.zero();
...
#pragma message "Using REGULAR CGRXU LOOKUPS"
                        ...
                        // after you're done with sorted probes for this step/phase
                        d_aux.free();
                        d_sorted_keys.free();
                        d_perm_sorted.free();
                        d_perm_in.free();
                ...
                if (active_range_end <= key_generation_size)
                {
                  ...
                  /*  if (totalleftover_keys < 20 && !alternate_updates)
                    {
                        ...
                        printf(" SKIPPING  miss 2 checks due to small leftover key size \n");
                    } */
                    ...
                    if (!skip_miss2_checks)
                    {
                        ...
                        Draw_Miss_Probes_From_Two_Ranges(
                            probe_size_misses,
                            generated_keys,
                            build_size,
                            key_generation_size,
                        ...
                        std::cerr << "        - Probe Inner Range for Exclusion Misses: [" << range_start << ", " << range_end << ")\n";  
...
#pragma message "Using REGULAR CGRXU SUCSSOR MISS LOOKUPS"
                                ...
                                { // Verification Block
                                    ...
                                    assert_ceiling_results_include_range<key_type>(
                                        a_sorted_miss_keys,
                                        b_succ_miss_keys,
                                        generated_keys,
                                        build_size,
