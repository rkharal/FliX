    ...
    size_t runs)
{
    ...
    for (size_t tci = 0; tci < test_configuration_options.size(); ++tci)
    {
        ...
        size_t build_size = (size_t{1} << tc.build_size_log) +10; // + 100;
            ...
            probe:;
                ...
                if (active_range_end > active_range_start)
                {
                    ...
                    if (sort_probes)
                    {
                        ...
                        C2EX
                                std::cerr
                            << "Sort: " << sort_time_ms << "ms-" << " probe_size: " << probe_size << std::endl;
...
#pragma message "Using BASELINE UNSORTED LOOKUP in LOOKUPS"
                ...
                {
                    std::cerr << "   --- XXXXXSorting first miss active range start " << active_range_start << ", active range end " << active_range_end << std::endl;
...
#pragma message "Using BASELINE SORTED LOOKUP for MISS CHECKS"
                ...
                */
                ...
                std::cerr << "   TOP ----> Sorting 2nd miss probes: active range start " << active_range_start << ", active range end " << active_range_end << std::endl;
                ...
                if (active_range_end < key_generation_size)
                {
                    ...
                    smallsize probe_size_misses = probe_size; // 16; //static_cast<smallsize>(key_generation_size - active_range_end);
                    std::cerr << "  ----> Sorting 2nd miss probes: active range start " << active_range_start << ", active range end " << active_range_end << " probe size misses " << probe_size_misses << std::endl;
                    std::cerr << "Drawing second miss probes Generated key size " << key_generation_size << std::endl;
                    draw_probes(probe_size_misses, generated_keys, active_range_end, key_generation_size, supports_updates, probe_keys_misses, expected_result_misses);

                    std::cerr << "Uploading second miss probes \n"
                              << std::endl;

                    probe_keys_buffer.upload(probe_keys_misses.data(), probe_size_misses);
                    result_buffer.zero();

                    // print all probe data in probe_keys_misses
                    //    for (size_t i = 0; i < probe_keys_misses.size(); ++i)
                    //    {
                    //        printf("Misses only Probe Keys:  %zu: %u \n", i, probe_keys_misses[i]);
                    //    }
                    ...
                    std::cerr << "active range end " << active_range_end
                    ...
                    if (sort_probes && (probe_size_misses > 0))
                    {
                        ...
                        // print out d_sorted_keys prior to sort
                        //  auto d_sorted_keys_host = d_sorted_keys.download(probe_size_misses);
                        //  for (size_t i = 0; i < probe_size_misses; ++i) {
                        //      printf("Before Sort - D Sorted Keys:  %zu: %u \n", i, d_sorted_keys_host[i]);
                        //  }
                        ...
                        /*
                                                ...
                                                key_only_sort_device_timed<key_type>(probe_keys_buffer, probe_size_misses,
                                                                                     ...
                                                                                     &sort_time_ms_local, 0);
                                                //sort_time_ms = sort_time_ms_local;
                                                //std::cerr << "      Sort: " << sort_time_ms << " ms - ";

                                                cudaStreamSynchronize(0);
                                                C2EX

                        */
                        cudaStream_t stream = 0; // or your existing stream variable

                        // --- Timed key-only sort ---
                        double sort_time_ms_local = 0.0;

                        // probe_keys_buffer must be: const cuda_buffer<key_type>& containing at least n elements
                        key_only_sort_device_timed<key_type>(
                            probe_keys_buffer,   // IN: device keys
                            probe_size_misses,   // n
                            d_sorted_keys,       // OUT: sorted keys
                            d_perm_sorted,       // OUT: permutation indices
                            d_aux,               // IN/OUT: scratch
                            &sort_time_ms_local, // OUT: elapsed ms (GPU)
                            stream               // stream
                        );

                        // If you need to use results on CPU, sync the stream (not necessarily whole device)
                        cudaStreamSynchronize(stream);
                        C2EX

                            // print out d_sorted_keys after the sort
                            /*
                                   auto d_sorted_keys_host_after = d_sorted_keys.download(probe_size_misses);
                               for (size_t i = 0; i < probe_size_misses; ++i)
                               {
                                   printf("After Sort - D Sorted Keys:  %zu: %u \n", i, d_sorted_keys_host_after[i]);
                               }. */

                            // std::cerr << "Probe Missing Keys Two:  ";
                            timer.start();
#ifdef BASELINES
