...
#pragma message("VALUE OF DIV=" TOSTRING(DIV))
...
public:
    ...
    size_t gpu_resident_bytes()
    {
        ...
        return copy_buffer.size_in_bytes() + as_buffer.size_in_bytes() + ordered_node_pairs_buffer.size_in_bytes() + (total_nodes_used_from_AR)*node_stride + maxvalues_buffer.size_in_bytes() + launch_params_buffer.size_in_bytes();
    }
    ...
    void data_structure_stats()
    {
        ...
        DATA_STRUCTURE_STATS("More Build params", 5, cache_line, node_size, partition_size, node_stride, percentage_initial_fill);
        return; // total_nodes_used_from_AR > total_allocation_region_nodes;
    }
...
#pragma message "TILE INSERTS_C_=YES"
            ...
            update_kernel_tile<key_type><<<maxblocksplus1, MAXBLOCKSIZE / DIV, 0, stream>>>(launch_params_buffer.ptr(), false);
...
#pragma message "TILE INSERTS=NO"
            update_kernel<key_type><<<SDIV(total_nodes, NUMTHREADS / DIV), NUMTHREADS / DIV, 0, stream>>>(launch_params_buffer.ptr(), false);
#endif
...
//printf("TILE DELETES=YES\n");
            ...
            smallsize blocksPerGrid = partition_count_with_overflow * TILE_SIZE; // NOT divided by anything
            smallsize maxblocksplus1 = SDIV(blocksPerGrid, MAXBLOCKSIZE / DIV) + 1;
            update_kernel_tile<key_type><<<maxblocksplus1, MAXBLOCKSIZE / DIV, 0, stream>>>(launch_params_buffer.ptr(), true);
...
#pragma message "TILE DELETES=NO"
            ...
            update_kernel<key_type><<<SDIV(total_nodes, NUMTHREADS / DIV), NUMTHREADS / DIV, 0, stream>>>(launch_params_buffer.ptr(), true);
#endif
        }
    }
...
#pragma message "LOOKUP_KERNEL_TILE=YES"
        ...
        lookup_kernel_tile_loop<key_type><<<SDIV(size, NUMTHREADS / DIV), NUMTHREADS / DIV, 0, stream>>>(launch_params_buffer.ptr(), group_lookup_kernel);
        ...
        lookup_kernel<key_type><<<SDIV(size, NUMTHREADS / DIV), NUMTHREADS / DIV, 0, stream>>>(launch_params_buffer.ptr(), group_lookup_kernel);
#endif
    }
